...
// function to add item into stack
void push(int item) { 
   stack[++top] = item; 
} 

// function to delete item from the stack
int pop() { 
   return stack[top--]; 
} 
...
void traverse(){
    ...
    while(count<v){
        ...
        //select the top element to traverse
        int vertex = pop();    
        
        for(int i=v-1; i>0; i--){
        
        //check if the vertex has any adjacent vertices which is not visited.
        //if there then add into the stack.
...
void main(){
    ...
    // function implementing dfs approach
    traverse();
